...
public class DateUtils {
    ...
    /*
      //拿到当前时间是一周的第几天
        ...
        if (dayOfWeek == 1){    //第一天表示星期天
            cal.add(Calendar.DATE,-6); //第1天表示星期天需要向前移动6天
        }else {
            cal.add(Calendar.DATE,2-dayOfWeek);  //其他的天数在本周中移动
        }
        System.out.println(DateUtils.parseDate2String(cal.getTime()));
     */
     ...
     *         int dayOfWeek = cal.get(Calendar.DAY_OF_WEEK);
     *         if (dayOfWeek != 1) //不是周日
     *             cal.add(Calendar.DATE, 8 - dayOfWeek);  //其他的天数在本周中移动
     */
